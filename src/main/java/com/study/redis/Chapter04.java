...
public class Chapter04 {
    ...
    public void testListItem(Jedis conn, boolean nested) {
        ...
        Set<String> i = conn.smembers("inventory:" + seller);//返回集合内所有的元素
    ...
    public boolean listItem(
        ...
        String inventory = "inventory:" + sellerId;//拼写集合名称
        String item = itemId + '.' + sellerId;//拼写成员 商品.商品id
        long end = System.currentTimeMillis() + 5000;

        while (System.currentTimeMillis() < end) {
            conn.watch(inventory);//监视用户包裹发生的变化
            if (!conn.sismember(inventory, itemId)){//检查商品是否在
                conn.unwatch();
                return false;
            }
