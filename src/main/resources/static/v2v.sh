#!/bin/bash
#wget https://api.github.com/repos/v2ray/v2ray-core/releases/latest
#latest_version=`grep tag_name latest| awk -F '[:,"v]' '{print $6}'`
#wget -c https://github.com/v2ray/v2ray-core/releases/download/v${latest_version}/v2ray-linux-64.zip
#unzip -oq v2ray-linux-64.zip -d /usr/bin/v2ray && cd /usr/bin/v2ray
#sudo mkdir /etc/v2ray /var/log/v2ray
#touch /etc/v2ray/config.json
#sudo mv systemd/v2ray.service /etc/systemd/system/
#sudo systemctl enable v2ray # 设置开机启动 用于程序开机启动并且崩溃时自动重启程序
...
installV2Ray(){
    ...
    if [ ! -f '/etc/v2ray/config.json' ]; then
        ...
        sed -e "s/10086/${PORT}/g; s/81ad6b10-8d1a-40f7-8ad041918-e3e35cd38297/${UUID}/g;" - > \
